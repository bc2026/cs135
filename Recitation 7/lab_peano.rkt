...
#| Now we can inductively define the peano numbers:
 ...
 |   "natural" refers to a nonnegative integer, and: {0, 1, 2, 3,...}
...
;; 3 PTS
(define (p-primeHelper? x y)
  (cond ((p= y x) #t)
        ((p-divides? y x) #f)
        (else (p-primeHelper? x (succ y)))))

(define (p-prime? p)
  (p-primeHelper? p (succ(succ z)))
)
