...
(define (intersection S1 S2)
  ...
  (define (intersection-helper set1 set2 result)
    (cond ((null? set1) result)
          ...
          (else (intersection-helper (cdr set1) set2 result))))
  
  (reverse (intersection-helper S1 S2 '())))
...
(define (disjoint? S1 S2)
  ...
  (define (check-disjoint set1 set2)
    (cond
          ((null? set1) #t)
          ((element? set2 (car set1) ) #f)
          (else (check-disjoint (cdr set1) set2))))
...
;; 3 PTS
(define (subset? S1 S2)
    (define (check-subset set1 set2)
    (cond ((null? set1) #t)
          ((element? set2 (car set1))
           (check-subset (cdr set1) set2))
          (else #f)))
...
(define (set-equal? S1 S2)
  (cond
    ...
    ((element? S2 (car S1) )
     (set-equal? (remove S1 (car S1)) (remove S2 (car S1) )))
    (else #f)))
...
(define (sym-diff S1 S2)
  (cond ((and (null? S1) (null? S2)) '())
        ...
        ((null? S2) S1)
        ((member (car S1) S2)
         (sym-diff (cdr S1) (remove S2 (car S1))))
...
(define (symmetric-closure relation)
  (define converse-relation (converse relation))
  (union relation converse-relation))
