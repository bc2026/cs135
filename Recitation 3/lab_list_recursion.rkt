...
(define (rotate c)(
  ...
  list (cons (car c) (cdr c))
        (cons (append (cdr c) (list (* -1 (car c)))) (list (* -1 (car (cdr c)))))
        (cons (* -1 (car c)) (list (* -1 (car (cdr c)))))
        (cons (* -1 (car (cdr c))) (list (car c))))
...
(define (combine-courses s1 s2)
  (append (get-courses s1) (get-courses s2))
)
...
(define (gen-fib a b n)
  (define (gen-fib-helper a b n f0 f1)
    (if (= n 0)
        f0
        (gen-fib-helper b (+ a b) (- n 1) f1 (+ a b))))
  (gen-fib-helper a b n a b))
